...
function edButton(id, display, tagStart, tagEnd, style, title) {
	this.id = id;			// used to name the toolbar button
	this.display = display;		// label on button
	this.tagStart = tagStart; 	// open tag
	this.tagEnd = tagEnd;		// close tag
	//this.open = open;		// set to -1 if tag does not need to be closed
	this.style = style;		// button style
	this.title = title;		// button name
}
...
function edShowButton(button, i) {
	if (button.id == 'ed_link') {
		document.write('<input type="button" id="' + button.id + '" class="ed_button" onclick="edInsertLink(edCanvas, ' + i + ');" value="' + button.display + '" style="' + button.style + '" title="' + button.title + '" />');
	}
	else if (button.id == 'ed_img') {
		document.write('<input type="button" id="' + button.id + '" class="ed_button" onclick="edInsertImage(edCanvas);" value="' + button.display + '" title="' + button.title + '" />');
	}
	else {
		document.write('<input type="button" id="' + button.id + '" class="ed_button" onclick="edInsertTag(edCanvas, ' + i + ');" value="' + button.display + '" style="' + button.style + '" title="' + button.title + '" />');
	}
}
...
function edToolbar() {
	...
	var count = edButtons.length;
	
	for (i = 0; i < count; i++) {
		edShowButton(edButtons[i], i);
	}
	document.write('<input type="button" id="ed_close" class="ed_button" onclick="edCloseAllTags();" value="Close Tags" title="Close Tags" />');
	document.write('</div>');
}
...
function edInsertImage(myField) {
	...
	var alt = prompt('Enter the image description (alt)', '');
